...
### PowerShell blocks `npm` with `npm.ps1 cannot be loaded`
You hit this exact Windows policy error:
...
1) **No system change (quickest): use `npm.cmd` directly**
...
2) **Allow local scripts for your user (recommended for dev machines)**
...
Close/reopen PowerShell, then test:
...
3) **One-session bypass (temporary)**
...
If you’re on a locked-down work PC, option 1 (`npm.cmd`) is usually the safest.
